...
仍建议下一步由浏览器目视确认 `/calendar/audit` 上的徽标、单 PAP `—`、抽奖详情跳转链接，以及新建负数 lottery 样本时的“消费 PAP”文案。
...
2. 阶段 2–5 已由用户反馈完成验证；阶段 6 已完成只读/控制器复核，后续优先补齐浏览器目视确认：
   - 审查页徽标 / 单 PAP `—` 展示 / 抽奖详情跳转链接
   - 新建或保留一个未清零、未退款的负数 lottery 样本，确认总额列显示“消费 PAP”文案
