...
- **当前总体结论**：
  ...
  - 阶段 7 已进入统计口径整理：设计从 earned / spent / balance 技术拆分，调整为「出勤 PAP / 消费 PAP / 当前可用 PAP」业务口径；默认军团贡献排行建议按出勤 PAP 排，抽奖 / 商店余额继续按当前可用 PAP 判断。
...
- 阶段 7 统计口径代码落地：个人 / 军团页面仍需按「出勤 PAP / 消费 PAP / 当前可用 PAP」拆分展示。
- 军团 PAP 统计 / 导出语义调整：默认贡献排行应按出勤 PAP 排，表格和 CSV 建议增加消费 PAP、当前可用 PAP。
- 自动开奖
- 到期自动退款
- 手动选号
...
1. 打开 `docs/PAP统计口径调整计划.md`，先确认阶段 7 的「出勤 PAP / 消费 PAP / 当前可用 PAP」口径。
2. 打开 `docs/PAP超网抽奖宿主验证清单.md`。
3. 阶段 2–6 已完成验证，暂无已知阻塞项；后续如新建或保留一个未清零、未退款的负数 lottery 样本，可顺手确认总额列显示“消费 PAP”文案。
4. 阶段 7 代码落地前，建议先在测试服只读巡检：普通行动是否存在最终负值、抽奖行动是否存在最终正值、`出勤 PAP - 消费 PAP` 是否等于当前 `SUM(value)`。
5. 阶段 3.5 的 30 角色并发购买已验证通过，可按需复现。
6. 如需回归抽奖主流程，可抽样复测：
   - 创建一条小抽奖
