...
- **待继续实测**：
  ...
  - 阶段 7 代码落地前，应先按 `docs/PAP统计口径调整计划.md` 做只读数据巡检：普通行动是否存在最终负值、抽奖行动是否存在最终正值、`出勤 PAP - 消费 PAP` 是否等于当前 `SUM(value)`。
...
- **已知现象（非缺陷，阶段 7 处理）**：抽奖消费是负数 `paps.value`，会即时进入角色 / 军团 PAP 统计页，使统计把「出勤 PAP」「消费 PAP」「当前可用 PAP」混在一起。这是 `docs/PAP统计口径调整计划.md` / 阶段 7 专门要处理的，目前按决策延后。
- 阶段 7 预期口径：普通行动最终认可正向值计入「出勤 PAP」，抽奖 / 商店等消费型行动当前净消耗计入「消费 PAP」，所有 `paps.value` 求和继续作为「当前可用 PAP」。
- 后续军团贡献排行默认应按「出勤 PAP」排序；抽奖购买与商店余额判断继续按「当前可用 PAP」。
