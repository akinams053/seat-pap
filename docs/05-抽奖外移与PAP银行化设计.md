...
> ⚠️ **部分决策已被 [`07-抽奖重做为行动设计.md`](07-抽奖重做为行动设计.md) 推翻（2026-06-03）**：抽奖不再是"月度消费锚 + 实时扣 + 外部消费审查页"，而是**恢复成「行动」**（一场 = 一个 operation、开奖人负责、进行动审查）。本文档的**银行 API（余额查询 / 写 token / JWT 跳转）、`is_consumption` 口径、商店实时扣**仍有效；但 **§2 抽奖建模、§7.3 消费锚、§9.3 外部消费审查页**已被 07 取代——读本文档前先看 07。
>
