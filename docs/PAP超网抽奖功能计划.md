...
PAP 余额要随抽奖消费减少，但阶段 7 统计页面应统一区分三类业务数值：

- **出勤 PAP**：成员通过普通行动最终被认可的 PAP，包含普通行动基础 PAP 与行动审查额外奖励，扣除普通行动审查扣罚 / 清零后的最终认可值；不包含抽奖或商店消费。
- **消费 PAP**：抽奖行动消耗，未来也可扩展到商店重启后的商品消费；阶段 7 第一版采用当前净消费语义，已退款或已撤销的消费不再计入。
- **当前可用 PAP**：剩余可继续用于抽奖 / 商店消费的 PAP，业务上等于出勤 PAP 减去消费 PAP。

第一版实现时，购买节点产生的负数 PAP 会进入 `kassie_calendar_paps.value` 汇总，因此现有 PAP 商店 API 与余额类统计会自然减少。

> 阶段 7 的查询层应优先通过 `kassie_calendar_lotteries.operation_id` 识别抽奖 operation：普通行动正向最终值计入出勤 PAP，抽奖 / 商店等消费型行动负向最终值计入消费 PAP，所有 `paps.value` 求和仍作为当前可用 PAP。
...
第一版不做“到期未售完自动退款”；但**允许**在 `open` 状态下、已有至少 1 个已售节点时，由 FC / 管理员按已售节点手动「提前开奖」。
...
- 每个奖品抽出一个中奖节点。
- 同一成员是否可重复中奖为每期可配置项。
- 开奖不自动执行，由 FC / 管理员手动点击。节点售完（`sold_out`）后可开奖；凑不满人时，也可在 `open` 状态下、已有至少 1 个已售节点的前提下「提前开奖」。
...
> 售满开奖与凑不满人时的「提前开奖」共用同一开奖流程，区别仅在于：提前开奖允许在 `open` 状态触发（需已有至少 1 个已售节点），开奖时未售节点作废（`voided_at`），`draw_log.draw_mode` 记为 `early`。下述编号步骤以售满开奖为例。
...
第一版不设置抽奖到期时间，因此不存在“到期自动退款”。未售完时，如已有至少 1 个已售节点，可由 FC / 管理员手动「提前开奖」；否则只能继续购买或取消退款。
...
抽奖消费进入 PAP 汇总后，军团 PAP 页面需要同步调整展示语义，避免「出勤贡献」和「消费扣减」混在一起看不清。

#### 军团 PAP 页面

阶段 7 建议用军团级汇总卡片 / 图表明确三类业务口径：

```text
出勤 PAP = 普通行动最终认可 PAP（排除抽奖 operation）
消费 PAP = 抽奖 / 商店等消费型 operation 的当前净消耗，展示时转为正数
...
- 支持按月查看：展示某月军团出勤 PAP、消费 PAP、当前可用 PAP 变化。
- 支持按整年查看：展示某年累计出勤 PAP、消费 PAP、当前可用 PAP 变化。
- 默认可显示从 `2026-01-01` 至今的累计总览。

实现上应通过 lottery 表关联识别抽奖 operation：

```text
出勤 PAP：排除 lotteries.operation_id 对应的 paps，并只统计普通行动最终正向值
消费 PAP：只统计 lotteries.operation_id 对应的负数 paps，并以正数展示
当前可用 PAP：继续使用所有 paps.value 的合计
```

#### 军团 PAP 排名导出

军团 PAP 排名导出 Excel / CSV 时，也要包含所选时间范围内的消费 PAP 和当前可用 PAP。
...
| 出勤 PAP | 所选时间范围内普通行动最终认可 PAP |
| 消费 PAP | 所选时间范围内抽奖 / 商店等消费型 operation 的当前净消耗，正数展示 |
| 当前可用 PAP | 出勤 PAP - 消费 PAP |
...
```text
出勤 PAP = 普通行动最终认可 PAP
消费 PAP = 抽奖 / 商店等消费型 operation 当前净消耗，展示为正数
当前可用 PAP = 出勤 PAP - 消费 PAP
```

当前抽奖可通过 lottery 表关联识别；未来商店重启后，建议为商店消费补充明确的 operation 类型或结构化 source，避免仅靠文案判断。
...
- `sold_out` 可正常开奖；`open` 且已有至少 1 个已售节点时可提前开奖。
- 已 `drawn` 的抽奖不能再次开奖。
...
`sold_out` 显示「开奖」按钮；`open` 且已有至少 1 个已售节点时显示「提前开奖」按钮。一个节点都没售出时不能开奖，只能继续购买或由 FC / 管理员取消并退款。
...
售满后正常开奖 `draw_mode` 为 `sold_out`；凑不满人时 FC 提前开奖 `draw_mode` 为 `early`，此时未售节点在开奖时作废（`voided_at`），不参与抽取。
...
### 阶段 7：PAP 统计口径与导出调整（后续阶段，不纳入第一版）

> 已决策：第一版先把抽奖创建 / 购买 / 开奖 / 退款 / 审查主流程跑通，本阶段延后到主流程稳定后再做。阶段 7 口径以 `docs/PAP统计口径调整计划.md` 为准。

1. 个人 / 军团 PAP 页面统一展示「出勤 PAP / 消费 PAP / 当前可用 PAP」。
2. 军团贡献排行默认按出勤 PAP 排，消费 PAP 与当前可用 PAP 作为辅助列展示。
3. 支持按月 / 按年查看军团出勤 PAP、消费 PAP、当前可用 PAP。
4. 军团 PAP 排名导出增加「出勤 PAP」「消费 PAP」「当前可用 PAP」列。
5. 导出中的消费 PAP 必须与页面筛选时间范围一致。
6. 代码落地前先只读巡检普通行动最终负值、抽奖行动最终正值等边界数据。
...
军团 PAP 页面阶段 7 应优先调整贡献排行与导出：默认按出勤 PAP 排，消费 PAP 和当前可用 PAP 作为辅助列；排名导出也必须带上相同时间范围内的消费 PAP 与当前可用 PAP。
...
- 个人 / 军团 PAP 页面的「出勤 PAP / 消费 PAP / 当前可用 PAP」三列统计改造（§7 阶段，延后到主流程跑通后再做）；
- 军团 PAP 排名导出增加出勤 PAP、消费 PAP、当前可用 PAP 列（同上，随阶段 7 一起做）；
- 手动选号；
- EVE Mail 自动通知；
- 自动发奖；
- 奖品库存管理；
